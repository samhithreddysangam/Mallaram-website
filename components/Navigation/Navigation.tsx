  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ease-in-out ${scrolled ? 'py-2' : 'py-6 md:py-10'}`}>
      <div className="max-w-7xl mx-auto px-4 md:px-6">
        <div className={`relative transition-all duration-500 bg-white/80 backdrop-blur-2xl border border-white/40 rounded-[2.5rem] px-4 md:px-6 lg:px-10 flex items-center justify-between shadow-[0_20px_50px_rgba(0,0,0,0.1)] ${scrolled ? 'h-14 md:h-16' : 'h-16 md:h-20'}`}>
          
          {/* LEFT: Village Name + Left Nav Items (flex-1 right-aligned to hug the logo) */}
          <div className="flex-1 flex items-center justify-end gap-1">
            <div className="hidden xl:flex flex-col items-end justify-center shrink-0">
              <span className="text-[10px] md:text-sm font-black text-[#0A0A0A] uppercase tracking-[0.1em] leading-none">{dictionary.hero.title}</span>
              <span className="text-[7px] md:text-[9px] font-medium text-[#15803d] uppercase tracking-[0.2em] mt-0.5">{dictionary.hero.tagline}</span>
            </div>
            <div className="hidden xl:flex items-center gap-0">
              {navItems.slice(0, 4).map((item) => (
                <Link
                  key={item.key}
                  href={item.href}
                  className="text-[9px] font-black text-[#0A0A0A]/60 hover:text-[#15803d] uppercase tracking-[0.12em] px-2 py-2 transition-all relative group whitespace-nowrap"
                >
                  {t(`nav.${item.key}`)}
          ...
          {/* CENTER: BIG LOGO */}
          <div className="flex-none flex justify-center relative z-10 mx-3 md:mx-5 lg:mx-6">
          ...
          {/* RIGHT: Right Nav Items + Controls (flex-1 left-aligned to hug the logo) */}
          <div className="flex-1 flex items-center justify-start gap-1">
            ...
            <div className="flex items-center gap-2 shrink-0 ml-auto">
              <button
                ...
                className="hidden md:block px-3 lg:px-4 py-2 text-[8px] lg:text-[9px] font-black rounded-xl bg-[#15803d]/10 text-[#15803d] hover:bg-[#15803d] hover:text-white transition-all border border-[#15803d]/20 uppercase tracking-widest active:scale-95"
              >
                {t('language.toggle')}
              </button>
              ...
              {session ? (
                <button
                  onClick={() => signOut()}
                  className="p-2 rounded-xl bg-gray-100 text-gray-500 hover:text-red-500 hover:bg-red-500/10 transition-all border border-gray-200"
                  title="Sign Out"
                >
                ...
                <Link
                  href={`/${locale}/login`}
                  className="px-4 lg:px-5 py-2 text-[8px] lg:text-[9px] font-black rounded-xl bg-[#15803d] text-white hover:bg-black transition-all uppercase tracking-widest shadow-lg shadow-[#15803d]/10 active:scale-95"
                >
                  {t('auth.login')}
                </Link>
              )}
              ...
              {/* Mobile Toggle */}
              <button
                className="xl:hidden p-2 rounded-xl bg-gray-100 text-[#0A0A0A] border border-gray-200"
