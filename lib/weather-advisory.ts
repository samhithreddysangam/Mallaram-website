...
function generateAdvisories(weather: WeatherData): { primary: Advisory; secondary: Advisory[] } {
  ...
  // --- PRIMARY ADVISORY: Most important action ---

  // 1. Extreme weather warnings take priority
  if (isVeryHot && isDry) {
    advisories.push({
      id: 'heatwave',
  ...
  // 2. Sowing conditions
    ...
    advisories.push({
      ...
      description: `Current temperature ${temp}°C and humidity ${humidity}% create ideal conditions for sowing. ${rainChance}% rain chance provides good moisture for germination. Recommended crops for current ${getSeason().season} season: ${getSeason().crop}.`,
      descriptionTe: `ప్రస్తుత ఉష్ణోగ్రత ${temp}°C మరియు తేమ ${humidity}% విత్తనాలు వేయడానికి అనువుగా ఉన్నాయి. ${rainChance}% వర్షం అవకాశం మొలకెత్తడానికి మంచి తేమను అందిస్తుంది. ప్రస్తుత ${getSeason().seasonTe} సీజన్‌కు సిఫార్సు చేసే పంటలు: ${getSeason().cropTe}.`,
      category: 'sowing',
      urgency: 'medium',
      icon: '🌱',
  ...
  // 7. General advisory
    ...
    advisories.push({
      ...
      description: `Mild conditions with ${temp}°C and ${humidity}% humidity. No rain expected (${rainChance}% chance). Good day for weeding, inter-culture operations, and field preparation for the next crop. Consult your local agriculture officer for crop-specific advice.`,
      descriptionTe: `${temp}°C మరియు ${humidity}% తేమతో సౌమ్య పరిస్థితులు. వర్షం అంచనా లేదు (${rainChance}% అవకాశం). కలుపు తీయడం, ఇంటర్-కల్చర్ మరియు తదుపరి పంటకు పొలం సిద్ధం చేయడానికి మంచి రోజు. పంట-నిర్దిష్ట సలహాల కోసం మీ స్థానిక వ్యవసాయ అధికారిని సంప్రదించండి.`,
      category: 'general',
      urgency: 'low',
      icon: '✅',
  ...
  // 8. Default: Mixed conditions advisory
  else {
    advisories.push({
      id: 'general-advisory',
      title: '📋 Daily Farm Advisory',
      titleTe: '📋 రోజువారీ వ్యవసాయ సలహా',
      description: `Current conditions: ${temp}°C, ${humidity}% humidity, ${rainChance}% rain chance. ${isWindy ? 'Strong winds present — delay spraying operations.' : ''} ${isHumid ? 'High humidity — monitor for pest activity.' : ''} Plan your farm work accordingly for the ${getSeason().season} season.`,
      descriptionTe: `ప్రస్తుత పరిస్థితులు: ${temp}°C, ${humidity}% తేమ, ${rainChance}% వర్షం అవకాశం. ${isWindy ? 'గాలులు వీస్తున్నాయి — పిచికారీ వాయిదా వేయండి.' : ''} ${isHumid ? 'అధిక తేమ — తెగుళ్ల కోసం గమనించండి.' : ''} ${getSeason().seasonTe} సీజన్ కోసం మీ వ్యవసాయ పనులను తదనుగుణంగా ప్లాన్ చేసుకోండి.`,
      category: 'general',
      urgency: 'low',
      icon: '📋',
  ...
  // SECONDARY ADVISORIES: Generate 1-2 additional contextual tips
  ...
  if (seasonInfo.season === 'Kharif (Monsoon)') {
    ...
    secondary.push({
      ...
      description: 'Rabi season is here. Prepare nursery beds for Rabi crops. Apply well-decomposed FYM (Farm Yard Manure) 2-3 weeks before sowing. Maintain proper spacing for better yield.',
  ...
  // Add weather-specific secondary advisory
...
export function getAiAdvisory(weather: WeatherData): AdvisoryResponse {
  const seasonInfo = getSeason();
  const { primary, secondary } = generateAdvisories(weather);
